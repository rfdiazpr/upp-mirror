...
public:
	...
	T&       AddPick(const K& k, T pick_ x)   { key.Add(k); return value.AddPick(x); }
	...
	FixedAMap(Vector<K> pick_  key, V pick_ val) : key(key), value(val) {}
...
public:
	...
	FixedVectorMap(Vector<K> pick_  key, Vector<T> pick_ val) : FixedAMap<K, T, Vector<T>, Less>(key, val) {}
	FixedVectorMap()                                                       {}
...
public:
	...
	FixedArrayMap(Vector<K> pick_ ndx, Array<T> pick_ val) : FixedAMap<K, T, Array<T>, Less>(ndx, val) {}
	FixedArrayMap() {}
