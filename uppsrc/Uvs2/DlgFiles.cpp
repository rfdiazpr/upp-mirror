...
struct GapDisplayCls : public Display {
	virtual void Paint(Draw& draw, const Rect& rc, const Value& v, Color ink, Color paper, dword style) const
	{
		...
		draw.Clip(clip);
		Display::Paint(draw, clip, v, ink, paper, style);
		draw.End();
	}
};
...
void TextCompareCtrl::Paint(Draw& draw)
{
	...
	int first_line = max((clip.top + offset.cy) / letter.cy, 0);
	int last_line = min(idivceil(clip.bottom + offset.cy, letter.cy), lines.GetCount() - 1);
	...
	draw.Clip(number_width, clip.top, clip.right - number_width, clip.Height());
	for(int i = first_line; i <= last_line; i++) {
		const Line& l = lines[i];
		int y = i * letter.cy - offset.cy;
		draw.DrawRect(clip.left, y, clip.Width(), letter.cy, SWhite());
		draw.DrawText(number_width - offset.cx, y, ExpandTabs(l.text), l.level == 1 ? ifont : font, l.color);
	}
	draw.End();
	int lcy = (last_line + 1) * letter.cy - offset.cy;
	draw.DrawRect(clip.left, lcy, clip.Width(), clip.bottom - lcy, SGray());
}
...
void TextCompareCtrl::SetFont(Font f, Font nf)
{
	font = f;
	number_font = nf;
	FontInfo fi = ScreenInfo().GetFontInfo(f);
	FontInfo ni = ScreenInfo().GetFontInfo(nf);
...
{
	scroll.Set(scroll, (scroll.GetReducedViewSize() - Size(number_width, 0)) / letter, Size(maxwidth, lines.GetCount()));
	Refresh();
}
...
//		TOOL(SyncUpload)
	...
	void                              OpenFTP(FtpClient& ftp, bool retry = false);
	void                              LogLine(String line);
	int                               GetLocalBlock();
	UvsHostIndex                      GetHostIndex(FtpClient& ftp);
	void                              LockHost(FtpClient& ftp);
	void                              SaveLockFile(FtpClient& ftp);
	Array<UvsDataBlock>               Download(FtpClient& ftp, int first, int last);
	void                              RemoveLocal();
	void                              SaveIndexFile(FtpClient& ftp, const UvsHostIndex& hostindex);

	void                              SaveRetry(FtpClient& ftp, String file, String data, Gate2<int, int> progress = false);
	String                            LoadRetry(FtpClient& ftp, String file, bool allow_void, Gate1<String> progress = false);
	void                              DeleteRetry(FtpClient& ftp, String file);

	void                              RemoveDeep(String path);
	...
	void                              SetProgressSize(int size, int total);
...
UvsHostIndex DlgFiles::GetHostIndex(FtpClient& ftp)
{
	String idxfn = "uvs.idx";
	String index = LoadRetry(ftp, idxfn, true);
	return UvsHostIndex::Decompress(index);
}

Array<UvsDataBlock> DlgFiles::Download(FtpClient& ftp, int first, int last)
{
	...
	for(int i = first; i <= last; i++) {
		...
		bool saved = true;
		if(IsNull(file)) {
			file = LoadRetry(ftp, path, false, THISBACK(OnLoadGateAll));
			saved = false;
		}
...
void DlgFiles::OpenFTP(FtpClient& ftp, bool retry)
{
	ftp.Close();
	if(IsNull(job.host))
	...
	int rn = 0;
	connect_time = msecs();
	while(!ftp.Connect(job.host, job.user, job.password, job.passive)) {
		if(++rn > RETRIES)
			throw Exc(NFormat(t_("Error connecting to server '%s', user '%s' (%[0:active;passive]s)."), job.host, job.user, job.passive));
		ConLine(NFormat(t_("Error connecting to '%s', retrying (%d of %d); FTP error: %s"), job.host, rn, RETRIES, ftp.GetError()));
	}
	if(!ftp.Cd(job.hostrepository))
...
{
	FtpClient ftp;
	ftp.WhenProgress = THISBACK(OnFtpProgress);
	...
	try {
		...
		UvsHostIndex hostindex = GetHostIndex(ftp);
		...
		if(!has_lblock && !create) {
			...
			int idx = ScanInt(LoadRetry(ftp, "full.idx", true, THISBACK(OnLoadGateAll)));
			if(!IsNull(idx)) {
				String full = LoadRetry(ftp, "full.udb", false, THISBACK(OnLoadGateAll));
				if(!IsNull(full)) {
					loaded_full = true;
		...
		if(hostindex.last_update >= lblock) {
			Array<UvsDataBlock> down = Download(ftp, lblock, hostindex.last_update);
			while(!down.IsEmpty())
...
//					cfi.tree_time = (is_deleted ? int(Null) : cfi.prev.filetime);
		...
		if(operation != OP_DOWNLOAD && !job.anonymous) { // upload
			...
			for(int i = 0; i < cinfo.GetCount(); i++) {
				...
				if(
//				(confirm || todo.Find(i) >= 0) &&
				!job.IsExcluded(cfi.path) && cfi.IsUpload()) {
					if(count_upload == 0)
			...
			if(!out.download.IsEmpty()) {
				...
				if(job.fullblock) { // upload full block
					...
					SaveRetry(ftp, "full.udb", out, THISBACK(OnSaveGate));
					SaveRetry(ftp, "full.idx", FormatInt(hostindex.last_update));
				}
				...
				ConLine(bdesc);
				LogLine(bdesc);
				SaveRetry(ftp, fn, cblock, THISBACK(OnSaveGate));
				SaveIndexFile(ftp, hostindex);
			}
		...
		if(ftp.IsOpen() && is_locked) {
			ConAdd(t_("\nSynchronization aborted, unlocking server archive"));
			ConLine(ftp.Delete("lock") ? String(t_(" - OK"))
				: String(t_(" - error: ") + ftp.GetError()));
		}
		ConLine("\n" + e);
		ShowExc(e);
...
void DlgFiles::SetProgressSize(int size, int total)
{
	...
	progress.Text(String().Cat() << size << " / " << total << suffix);
	statusbar.Sync();
}

bool DlgFiles::OnLoadGateAll(String data)
{
	SetProgressSize(data.GetLength(), UvsDataBlock::GetLength(data, false));
	return IsCanceled();
}

bool DlgFiles::OnDecompressGate(int len, int total)
{
	SetProgressSize(len, total);
	return IsCanceled();
}

bool DlgFiles::OnSaveGate(int done, int total)
{
	SetProgressSize(done, total);
	return IsCanceled();
}
...
void DlgFiles::LockHost(FtpClient& ftp)
{
	if(ftp.MkDir("lock")) {
		ftp.RmDir("lock");
		SaveLockFile(ftp);
	...
	String lockfile = LoadRetry(ftp, "lock", true);
	if(!IsNull(lockfile)) {
		Time locktime;
		String lockauthor;
		try {
...
void DlgFiles::SaveLockFile(FtpClient& ftp)
{
...
void DlgFiles::DoSyncExport()
{
	...
	try {
		...
		SaveRetry(ftp, nb, out, THISBACK(OnSaveGate));
		SaveIndexFile(ftp, hostindex);
		SaveLocalDir(hostindex.last_update);
		is_locked = false;
		ConLine(t_("Unlocking server archive"));
		...
		if(ftp.IsOpen() && is_locked) {
			...
			ConLine(ftp.Delete("lock") ? String(t_(" - OK")) : String(t_(" - error: ") + ftp.GetError()));
		}
		ConLine("\n" + e);
		ShowExc(e);
	}
...
void DlgFiles::RemoveDeep(String path)
{
...
void DlgFiles::SaveIndexFile(FtpClient& ftp, const UvsHostIndex& hostindex)
{
	ConLine(t_("Saving index file"));
	int retry = 0;
	for(;;)
		try {
			SaveRetry(ftp, "uvs.new", hostindex.Compress());
			ftp.Delete("uvs.old");
			ftp.Rename("uvs.idx", "uvs.old");
			if(!ftp.Rename("uvs.new", "uvs.idx"))
...
void DlgFiles::SaveRetry(FtpClient& ftp, String file, String data, Gate2<int, int> progress)
{
	int retry = 0;
	for(;;) {
		this->progress.Text(NFormat(t_("Writing file: %s"), file));
		statusbar.Sync();
		if(ftp.Save(file, data, progress))
			return;
		if(IsCanceled())
			throw AbortExc();
		if(++retry > RETRIES)
			throw Exc(NFormat(t_("Error saving file '%s': %s"), file, ftp.GetError()));
		ConLine(NFormat(t_("Error saving file '%s', retrying (%d of %d); FTP error: %s"), file, retry, RETRIES, ftp.GetError()));
		ftp.Close();
		OpenFTP(ftp);
	}
}

String DlgFiles::LoadRetry(FtpClient& ftp, String file, bool allow_empty, Gate1<String> progress)
{
	int retry = 0;
	for(;;) {
		this->progress.Text(NFormat(t_("Reading file: %s"), file));
		statusbar.Sync();
		String s = ftp.Load(file, progress);
		if(IsCanceled())
			throw AbortExc();
		if(!s.IsVoid())
			return s;
		if(++retry > RETRIES) {
			if(allow_empty)
				return s;
			throw Exc(NFormat(t_("Error reading file '%s': %s"), file, ftp.GetError()));
		}
		if(!allow_empty)
			ConLine(NFormat(t_("Error reading file '%s', retrying (%d of %d); FTP error: %s"), file, retry, RETRIES, ftp.GetError()));
		OpenFTP(ftp, true);
	}
}

void DlgFiles::DeleteRetry(FtpClient& ftp, String file)
{
	int retry = 0;
	while(!ftp.Delete(file)) {
		if(++retry > RETRIES)
			throw Exc(NFormat(t_("Error deleting file '%s': %s"), file, ftp.GetError()));
		ConLine(NFormat(t_("Error deleting file '%s', retrying (%d of %d); FTP error: %s"), file, retry, RETRIES, ftp.GetError()));
		OpenFTP(ftp, true);
	}
}
