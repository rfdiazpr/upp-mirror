...
TabWindow* PaneFrame::AddTabWindow(DockableCtrl& ctrl)
{
	...
	if(tabwindow || HasTabWindow(reinterpret_cast<TabWindow*>(&ctrl)))
...
TabWindow* PaneFrame::FindTabWindow(int position)
{
	...
	for(int i = 0; i < tabwindows.GetCount(); i++)		
		if(tabwindows.At(i)->Position() == position &&
		tabwindows.At(i)->Alignment() == GetType())
			 return tabwindows.At(i);
	return NULL;
}
...
PaneFrame& PaneFrame::Create(int alignment, int size)
{
	created = true;
	maxsize = size;
	...
	{
		case LEFT:
			return Left(dockcontainer.SizePos(), size);
		case TOP:
			return Top(dockcontainer.SizePos(), size);
		case RIGHT:
			return Right(dockcontainer.SizePos(), size);
		case BOTTOM:
			return Bottom(dockcontainer.SizePos(), size);
	}
	return *this;
}
