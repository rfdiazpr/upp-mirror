...
Ctrl& DockCtrl::Dock(DockableCtrl& ctrl)
{
    ...
    if(!ctrl.IsFloating())
    {
        ...
        if(!paneframe.IsCreated() && alignment != DockableCtrl::DOCK_TABBED)
        {
            GetBase().AddFrame(paneframe.SetBase(this)
            .Create(alignment, 
            	alignment == DockableCtrl::DOCK_LEFT || 
            	alignment == DockableCtrl::DOCK_RIGHT ?
            	panesize.cx : panesize.cy));
        }
        switch(alignment)
        {
