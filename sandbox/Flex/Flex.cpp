...
template <class T>
void SetBench(T *rndpos, const char *s) {
	Vector<String> N = Split(s, ' ');
	for(int i = 0; i < N.GetCount(); i++) {
		int elements = atoi(N[i]);
	#ifdef _DEBUG
		int repeat = 1;
	#else
	...
	#endif
		int count1, count2;
		RLOG("------ " << elements << " elements " << repeat << "x");
		{
			TimeStop tm;
			String info;
		...
		}
		{
			...
			RLOG("Index FindAdd: " << tm.Elapsed() << "ms");
			Index<T> set;
			for(int i = 0; i < elements; i++)
				set.FindAdd(rndpos[i]);
			TimeStop tm2;
...
void SetBenchmark(const char *n)
{
	RLOG("========= int");
	SetBench<int>(rndpos, n);
	Buffer<String> rnd(10000000);
	for(int i = 0; i < 10000000; i++)
		rnd[i] = AsString(rndpos[i]);
	RLOG("========= String");
	SetBench<String>(rnd, n);
}
...
template <class T>
void InsertBench(const T& data, int from = 10, int to = 500000, bool compare = false)
{
...
void IterateBenchmark(int elements = 10000000)
{
	int N = 10;
	while(elements > 0) {
		RLOG(elements << "(" << N << "x)");
