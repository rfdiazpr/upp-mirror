...
protected:
	...
	void     Free();
	void     Pick(BiVector pick_ x) { vector = pick(x.vector); start = x.start; items = x.items;
...
public:
	...
	void     AddHeadPick(T pick_ x) { new(AddHead0()) T(x); }
	void     AddTailPick(T pick_ x) { new(AddTail0()) T(x); }
	...
	BiVector(BiVector pick_ src)                { Pick(pick(src)); }
	void operator=(BiVector pick_ src)          { Free(); Pick(pick(src)); }
...
public:
	...
	void     AddHeadPick(T pick_ x)       { bv.AddHead(new T(x)); }
	void     AddTailPick(T pick_ x)       { bv.AddTail(new T(x)); }
	...
	BiArray(BiArray pick_ src) : bv(pick(src.bv))  {}
	void operator=(BiArray pick_ src)        { Free(); bv = pick(src.bv); }
	BiArray()                                {}
	~BiArray()                               { Free(); }
