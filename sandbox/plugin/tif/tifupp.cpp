...
#include "lib/tiffconf.h"
#define	tif_int32 TIFF_INT32_T
#define	tif_uint32 TIFF_UINT32_T
...
*/
int
TIFFgetMode(const char* mode, const char* module)
{
#ifdef flagEMBED_TIF
	return _TIFFgetMode(mode,module);
#else
	int m = -1;
...
Index<tmsize_t> size_index;
Vector<int> size_alloc_calls, size_free_calls;
int op_id;

void dbgAddAlloc(void *p, tmsize_t s)
{
	++op_id;
	total_allocated += s;
...
void dbgAddFree(void *p, tmsize_t s)
{
	++op_id;
	total_freed += s;
	int i = size_index.Find(s);
...
#ifdef flagEMBED_TIF
extern "C" void* _TIFFmalloc(tmsize_t s)
{
	byte *p = new byte[s + 4];
	Poke32le(p, s);
#if DBGALLOC
...
extern "C" void    _TIFFfree(void* p)
{
	if(p) {
		byte *rawp = (byte *)p - 4;
#if DBGALLOC
...
extern "C" void* _TIFFrealloc(void* p, tmsize_t s)
{
...
static tmsize_t ReadStream(thandle_t fd, void* buf, tmsize_t size)
{
...
static tmsize_t WriteStream(thandle_t fd, void* buf, tmsize_t size)
{
	Stream *stream = reinterpret_cast<Stream *>(fd);
...
static int MapStream(thandle_t fd, void* *pbase, toff_t *psize)
{
	return 0;
}

static void UnmapStream(thandle_t fd, void* base, toff_t size)
{
}
...
struct ::tiff *TIFFFileStreamOpen(const char *filename, const char *mode)
{
	...
	int m = TIFFgetMode(mode, "TIFFOpen");
...
tmsize_t TIFEncoder::Data::ReadStream(thandle_t fd, void* buf, tmsize_t size)
{
...
tmsize_t TIFEncoder::Data::WriteStream(thandle_t fd, void* buf, tmsize_t size)
{
	Data& wrapper = *reinterpret_cast<Data *>(fd);
	ASSERT(wrapper.stream.IsOpen());
...
int TIFEncoder::Data::MapStream(thandle_t fd, void* *pbase, toff_t *psize)
{
	return 0;
}

void TIFEncoder::Data::UnmapStream(thandle_t fd, void* base, toff_t size)
{
}
