...
public:	
	...
	virtual void ChildAdded(Ctrl *child) 			{ if (!internal_block && child->GetParent() == this) AddRoot(*child, false); }
...
private:	
	struct Node
	{
		...
		Node() : parent(NULL), first(NULL), last(NULL), next(NULL), prev(NULL), vert(false), sz(10000)
			{ }		
	};
	...
	int 	BarSize() const							{ return 4; }
public:
	Ctrl &	AddSibling(bool before, Ctrl &sibling, Ctrl &c);
	Ctrl & 	AddSiblingOver(bool vert, bool before, Ctrl &sibling, Ctrl &c);
	Ctrl & 	AddSiblingUnder(bool vert, bool before, Ctrl &sibling, Ctrl &c);
	Ctrl & 	AddRoot(Ctrl &ctrl, bool first);
