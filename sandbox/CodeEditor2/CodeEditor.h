...
#undef HL_COLOR
	...
	static HlStyle& Get(int n) {return list[n];}
	static void Defaults();
	static void Set(int i, Color c, bool bold = false, bool italic = false, bool underline = false);

private:
	static HlStyle list[HL_COUNT];
};

class Highlighter : Moveable<Highlighter> {
	typedef Highlighter CLASSNAME;
private:
	friend class CodeEditor;
	
	struct Rule : public Moveable<Rule> {
		bool multi:1;
		bool sub:1;
		int font:3;
		RegExp begin, end;
		Color ink, paper;
		Rule() : multi(false), sub(false), font(0) {}
		Vector<int> subrules;
		String ToString() const { return Format("%s%s\n ink:%s, paper:%s, %d subrules",begin.GetPattern(),multi?"\n      "+end.GetPattern():String(),AsString(ink),AsString(paper),subrules.GetCount());}
	};
	...
	static VectorMap<String,Highlighter> list;
...
public:
	...
	int GetRulesCount() const                   {return rules.GetCount();}
	void LoadSyntaxFile(const char* fn);
	void LoadSyntax(const String& syntax, bool ignore_name = false);
	Rule& AddRule(const char* begin, const char* end, Color ink, Color paper, int font);
	Rule& AddRule(const char* pattern, int subrule, Color ink, Color paper, int font);
	void AddStyleRule(int n, const char* begin, const char* end);
	void AddStyleRule(int n, const char* pattern, int subrule = -1);
	void Scope(const char* start, const char* end);
	int Pos() const {
	...
	static int GetCount()                          {return list.GetCount();}
	
	Highlighter() {}
	Highlighter(String filename) {LoadSyntaxFile(filename);}
	~Highlighter() {}
};
...
protected:
//	virtual void HighlightLine_(int line, Vector<Highlight>& h, int pos);
...
public:
	typedef HlStyles::HlStyle HlStyle;
	
	//this is here only for bw compatibility
#define HL_COLOR(x, a, b)      x,
	enum {
#include "hl_color.i"
		HL_COUNT
...
//	int    GetHighlight() const       { return highlight; }
	...
	////////////////////////// CodeEditor2 specific //////////////////////////
private:
	struct Hl : public Moveable<Hl> {
		bool begin:1;
		unsigned rule:16;
		int level:16;
		unsigned pos:16;
		Hl() : begin(false), rule(0), level(0), pos(0) {}
		inline bool operator<(const Hl& x) const {
			return pos == x.pos ? rule < x.rule : pos < x.pos;
		}
		String ToString() const { return Format("%i R%i, L%i, %s",(int)pos,(int)rule,(int)level,String(begin?"B":"E")); }
	};
	
private:
	typedef Highlighter::Rule Rule;
public:
	void   Highlight(int h)              { highlight = h; cache.Clear(); Refresh(); }
	bool   Highlight(const String& name) { int n = Highlighter::Find(name); if(n<0) return false; Highlight(n); return true; }
	int    GetHighlight() const          { return highlight; } 
	
	struct Cache : public Vector<Hl>, public Moveable<Cache> {
		bool dirty;
		Cache() : dirty(true) {}
	};
	...
	const Hl& FindPartner(int ln, int i, int& ln2);
	const Hl& FindNext(int ln, int i, int& ln2);
	const Hl& FindPrev(int ln, int i, int& ln2);
...
protected:
	...
	void Recalculate(int ln, bool force = false);
	virtual void RefreshLine(int i);
	virtual void  HighlightLine(int ln, Vector<LineEdit::Highlight>& h, int pos);
};
