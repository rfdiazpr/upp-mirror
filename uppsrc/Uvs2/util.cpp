...
UvsDataBlock UvsDataBlock::Decompress(String data, bool headers_only, Gate2<int, int> progress)
{
	...
	int down_length = PeekIL(p);
	dword down_crc = PeekIL(p + 4);
	p += 8;
	...
	{
		total_length += (data_length[i] = PeekIL(p));
		data_crc[i] = PeekIL(p + 4);
		p += 8;
		if(version >= 2)
		{
			total_length += max(0, version_length[i] = PeekIL(p));
			version_crc[i] = PeekIL(p + 4);
			p += 8;
		}
	}
	int zdir_length = -1;
	if(version >= 2)
	{
		zdir_length = PeekIL(p);
		p += 4;
	}
	int zdown_length = PeekIL(p);
	int zdata_length = PeekIL(p + 4);
	p += 8;
	if(zdir_length > e - p)
...
int UvsDataBlock::GetLength(String data, bool headers_only)
{
	...
	if(memcmp(data, block_key, BLOCK_KEY_LENGTH))
		return 0;
	int version = PeekIL(~data + BLOCK_KEY_LENGTH);
	if(version > COMPRESS_VERSION)
		return 0;
	int hdrlen = BLOCK_KEY_LENGTH + (version <= 1 ? 6 : 9) * 4;
	if(l < hdrlen)
		return 0;
	int block_count = PeekIL(~data + BLOCK_KEY_LENGTH + 4);
	if(block_count < 0 || block_count >= MAX_BLOCK_COUNT)
		return 0;
	hdrlen += (version <= 1 ? 8 : 16) * block_count;
	if(l < hdrlen)
		return Null;
	int xlen = (version >= 2 ? PeekIL(~data + hdrlen - 12) : 0);
	int dlen = PeekIL(~data + hdrlen - 8);
	int blen = PeekIL(~data + hdrlen - 4);
