...
static int sMaskPos32(dword mask)
{
	if(mask == 0xff000000)
		return 3;
	if(mask == 0xff0000)
...
{
	rpos = sMaskPos32(rmask);
	gpos = sMaskPos32(gmask);
	bpos = sMaskPos32(bmask);
	apos = 0;
	type = RASTER_24;
}
...
{
	rpos = 2 - sMaskPos32(rmask);
	gpos = 2 - sMaskPos32(gmask);
	bpos = 2 - sMaskPos32(bmask);
	apos = 0;
	type = RASTER_24;
}
...
{
	rpos = 3 - sMaskPos32(rmask);
	gpos = 3 - sMaskPos32(gmask);
	bpos = 3 - sMaskPos32(bmask);
	if(amask) {
		apos = 3 - sMaskPos32(amask);
		type = RASTER_32ALPHA;
	}
	else
		type = RASTER_32;
...
{
	return (type & 31) == RASTER_32ALPHA &&
	#ifdef PLATFORM_WIN32
		bpos == 0 && gpos == 1 && rpos == 2 && apos == 3
	#else
	#error Implemenent!
	#endif
	;
...
{
	static byte bits[] = { 1, 2, 4, 8, 16, 24, 32, 32 };
	int b = bits[type & 15];
	return (cx * b + 7) / 8;
}

int RasterFormat::GetColors() const
{
	static int colors[] = { 2, 4, 16, 256, 65536, 256 * 256, 256 * 256 * 256, 256 * 256 * 256 };
	return colors[type & 15];
}

void RasterFormat::TailBits(RGBA *t, const byte *src, int cx, byte and, byte shift, const RGBA *palette)
{
	if(cx) {
		byte c = *src;
		while(cx > 0) {
			*t++ = palette[c & and]; c >>= shift;
			cx--;
		}
	}
}

void RasterFormat::TailBitsMSB1st(RGBA *t, const byte *src, int cx, byte shift1, byte and, byte shift, const RGBA *palette)
{
	if(cx) {
		byte c = *src;
		while(cx > 0) {
			*t++ = palette[(c >> shift1) & and]; c <<= shift;
			cx--;
		}
	}
}
