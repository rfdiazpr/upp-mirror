...
void CodeEditor::DirtyFrom(int line) {
	LOGFN;
	LDUMP(line);
//	for(int i = line; i < cache.GetCount(); i++)
//		cache[i].dirty = true;
	if(check_edited) {
...
void CodeEditor::InsertLines(int line, int count) {
	LOGFN;
	LDUMP(line);LDUMP(count);
	int pos=0;
	LDUMP(pos=GetCursor());LDUMP(GetCursorLine());LDUMP(GetPos(line));
	cache.InsertN(line, count);
	int extra=line?1:0;
	LDUMP(extra);
	for(int i = -extra; i < min(count+extra, limit); i++)
		Recalculate(line+i,true);
...
void CodeEditor::RemoveLines(int line, int count) {
	LOGFN;
	LDUMP(line);LDUMP(count);
	cache.Remove(line-1,count);
	if(line)
		cache[line-1].dirty = true;
...
void CodeEditor::IndentEnter(int count) {
	...
	LDUMP(count);
	LDUMPCC(cache);
	InsertChar('\n',count); // for now, just insert newline
//	cache.InsertN(GetCursorLine(),count);
	LDUMPCC(cache);
//TODO: Figure out what exactly this code should do and reimplement it
//	while(count--) {
//		int pp = GetCursor();
...
{
	HlStyles::Set(i,c,bold,italic,underline);
//	HlStyle& st = HlStyles::Get(i);
//	st.color = c;
//	st.bold = bold;
//	st.italic = italic;
