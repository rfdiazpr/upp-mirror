...
void GridCtrl::StdToolBar(Bar &bar)
{
	...
	if(removing)
		bar.Add(c, t_("Delete"), GridImg::ImgGridDel(), StdRemove);

	if(editing)
	{
		bar.Add(!ctrls && c, t_("Edit"), GridImg::ImgGridMod(), StdEdit);
		if(accepting)
			bar.Add(ctrls, t_("Accept"), GridImg::ImgGridCommit(), THISBACK(DoEndEdit));
		if(canceling)
			bar.Add(ctrls, t_("Cancel"), GridImg::ImgGridCancel(), THISBACK(DoCancelEdit));
	}
	...
	if(moving)
	{
		...
		bar.Add(c, t_("Move up"), GridImg::ImgGridMoveUp(), THISBACK(DoSwapUp));
		bar.Add(c, t_("Move down"), GridImg::ImgGridMoveDn(), THISBACK(DoSwapDown));
	}
...
ItemRect& GridCtrl::AddRow(int n, int size)
{
	Append0(n, size);	
	return GetRow();
}
...
int GridCtrl::Find(const Value &v, int col, int start_from)
{
...
bool GridCtrl::EndEdit(bool accept, bool doall)
{
	bool accepted = GetCtrlsData(curid.y, false, doall, accept);
	if(accept && !accepted)
		return false;
	...
	if(!accept)
	{
		canceled = CancelCtrlsData(curid.y);
...
bool GridCtrl::Remove0(int row, int cnt /* = 1*/, bool recalc /* = true*/, bool refresh /* = true*/, bool whens /* = true*/)
{
	...
	if(ready && recalc)
	{
		...
		if(refresh)
		{
			...
			RefreshFrom(row);
			if(x >= 0 && y >= 0)
				SetCursor0(x, max(fixed_rows, min(total_rows - 1, y)));
		}
	}
...
void GridCtrl::DoRemove()
{
	...
	else
	{
		...
		Refresh();
		curpos.y = -1;
		SetCursor0(curpos.x < 0 ? firstVisCol : curpos.x, max(fixed_rows, min(total_rows - 1, y)));
	}
