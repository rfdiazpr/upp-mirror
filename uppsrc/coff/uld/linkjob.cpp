...
//	mangling_style = MANGLING_MSC;
	...
	debug_info = false;
	show_logo = true;
	autocollect_crt_only = false; // VC71 fpinit!!!
//	cache_objects = false;
...
//		"   (default = on)\n"
		...
		"-dumpcomdats .. dump remapped COMDATs\n"
...
void LinkJob::ReadCommand(const Vector<String>& cmdlist, bool user)
{
	...
	if(user)
		user_command_args.Append(cmdlist);
	int start = msecs();
	for(int i = 0; i < cmdlist.GetCount(); i++) {
		...
		if(is_opt || *arg == '-') {
			...
			else if(lcmd == "alternatename") {
				int f = val.Find('=');
				if(f >= 0) {
					String name = val.Left(f), alt = val.Mid(f + 1);
					AddWeakExternal(Atomize(name), Atomize(alt));
				}
				else
					PutStdOut("'alternatename' option has invalid syntax (missing '=')");
			}
			else if(lcmd == "base")
			...
			if(eaten)
				...
				else if(lcmd == "dumpcomdats")
					dump_flags |= DUMP_COMDAT_REMAPS;
...
void LinkJob::SetEntryPoint()
{
	if(IsNull(entrypoint))
		...
		else if(subsystem == COFF_IMAGE_SUBSYSTEM_WINDOWS_GUI || subsystem == COFF_IMAGE_SUBSYSTEM_WINDOWS_CE_GUI)
...
//	int xxx = Atomize("__crtBreakAlloc");
	...
	for(int i = 0; i < objects.GetCount(); i++)
		if(objects[i].filetype == ObjectFile::DIRECT)
			CollectObject(i);
/* {
...
//			__asm int 3
		if(dump_flags & DUMP_COLLECTOR)
			PutStdOut(atoms[ref] + ": collecting symbol");
//		if(ref == xxx)
//			__asm int 3
...
*/					}
					...
					if(!matched) {
						PutStdOut(NFormat(
							...
							"-> used:      %s\n"
							"-> discarded: %s",
							atoms[ref], FormatSection(sym1.obj_sec), FormatSection(sym2.obj_sec)));
					}
				}
			}
			if(!inner_candidates.IsEmpty()) {
//				collected_symbols[i] = inner_candidates[0];
				...
				if(sym.obj_sec.object >= 0 && sym.obj_sec.section > 0)
...
*///				}
				if((sym.obj_sec.section || sym.obj_sec.object < 0) && collect_global < 0)
...
*/
	...
	for(int i = 0; i < collected_symbols.GetCount(); i++)
		if(collected_symbols[i] < 0) {
			...
			int anon = -1;
			while(f >= 0 && !globals[f].linked) {
				if(anon < 0) {
					ObjSec objsec = globals[f].obj_sec;
					if(objsec.section == 0 && objsec.object >= OBJ_FIRST)
						anon = f;
				}
			...
			if(f >= 0) {
				...
				while((g = globals.FindNext(g)) >= 0 && !globals[g].used)
					;
				if(g >= 0) {
					if(!(GetSection(globals[g].obj_sec).flags & GetSection(globals[f].obj_sec).flags
					& COFF_IMAGE_SCN_LNK_COMDAT)) {
						puts(NFormat("%s: duplicate\n"
							"-> used:      %s\n"
							"-> discarded: %s",
							atoms[atom], FormatSection(globals[f].obj_sec), FormatSection(globals[g].obj_sec)));
						globals[g].used = false;
					}
				}
				...
				Symbol& sym = globals[anon];
				ObjectFile& of = objects[sym.obj_sec.object];
				ObjectFile::Section& ofsec = of.sections[ObjectFile::SEC_ANON_COMDAT];
				int comdat_align = 1;
				while(comdat_align < 16 && !(sym.value & comdat_align))
					comdat_align <<= 1;
				int v = (ofsec.size + comdat_align - 1) & -comdat_align;
				ofsec.size = v + sym.value;
				of.used_any = ofsec.used = true;
				sym.value = v;
				sym.used = sym.linked = true;
				collected_symbols[i] = anon;
//				puts(NFormat("%s: anon comdat collect from %d", atoms[globals.GetKey(anon)], sym.obj_sec.object));
				fflush(stdout);
			}
		}
...
/*
void LinkJob::FixupAnonComdats()
{
	for(int i = 0; i < collected_symbols.GetCount(); i++) {
		atom_t atom = collected_symbols.GetKey(i);
		int iglo = collected_symbols[i];
		Symbol& sym = globals[iglo];
		if(sym.obj_sec.object >= OBJ_FIRST && sym.obj_sec.section == ObjectFile::SEC_ANON_COMDAT) {
			int f = -1;
			if(globals.FindPrev(iglo) >= 0 || globals.FindNext(iglo) >= 0) {
				String name = atoms[atom];
//				PutStdOut(NFormat("anon comdat: %s (%s)", name, FormatSection(sym.obj_sec)));
				for(f = globals.Find(atom); f >= 0; f = globals.FindNext(f))
					if(f != iglo) {
						const Symbol& sym = globals[f];
//						PutStdOut(NFormat("-> %s", FormatSection(sym.obj_sec)));
						if(sym.linked && sym.obj_sec.section > 0)
							break;
					}
			}
			if(f >= 0) { // remap to non-comdat
				if(dump_flags & DUMP_COMDAT_REMAPS)
					PutStdOut(NFormat("%s: COMDAT remapped to %s", atoms[atom], FormatSection(globals[f].obj_sec)));
				collected_symbols[i] = f;
				sym.used = false;
				globals[f].used = true;
			}
...
//					PutStdOut(NFormat("stub import[%d] = %s", of.used_stubs.GetCount(), atoms[globals.GetKey(g)]));
			...
			if(!of.used_imports.IsEmpty()) {
				...
				for(g = 0; g < of.used_imports.GetCount(); g++) {
					...
					if(oi.imp_atom) { // create hint/name table entry
						ival = namesec.size;
		...
		if(objects[i].used_any) {
			...
			for(int s = 0; s < of.sections.GetCount(); s++)
				if(of.sections[s].used) {
					const ObjectFile::Section& ofsec = of.sections[s];
...
//					if(se.udata)
//						puts(NFormat("UDATA[%08x], s0=%d: %s:%d", ofsec.flags,
...
{
	for(int f = weak_externals.Find(atom); f >= 0; f = weak_externals.FindNext(f))
		if(weak_externals[f] == refatom)
			return;
...
void LinkJob::PrepareImageHeader()
{
	...
	CatN(output_image, stub_data.GetLength(), stub_data);
	if(output_image.GetCount() < 0x40)
		output_image.SetCountR(0x40, 0);
	PokeIL(output_image.GetIter(0x3C), output_image.GetCount());
	CatN(output_image, 4, "PE\0\0");
	ifhdr_pos = CatN0(output_image, sizeof(COFF_IMAGE_FILE_HEADER));
	iohdr_pos = CatN0(output_image, sizeof(COFF_IMAGE_OPTIONAL_HEADER32));
...
//		has_relocs = true;
	...
	secmap_pos = CatN0(output_image, sizeof(COFF_IMAGE_SECTION_HEADER) * section_count);
//	impmap_pos = CatN0(output_image, sizeof(COFF_IMAGE_IMPORT_DESCRIPTOR) * (used_dll_objects.GetCount() + 1));
	header_size = output_image.GetCount();
	code_rfa = (header_size + file_align - 1) & -file_align;
	code_rva = (header_size + image_align - 1) & -image_align;
	output_image.SetCount(code_rfa + groups[GRP_COUNT].rfa, 0);
	memset(output_image.GetIter(code_rfa), 0x90, groups[GRP_DATA_BEGIN].rfa - groups[GRP_TEXT_BEGIN].rfa);
}
...
///			PutStdOut(NFormat("%s
	...
	for(i = 0; i < section_name_map.GetCount(); i++) {
		...
		memcpy(&output_image[secmap_pos + (int)sizeof(COFF_IMAGE_SECTION_HEADER) * i], &sechdr, sizeof(sechdr));
	}
...
//			| COFF_IMAGE_SCN_MEM_SHARED | COFF_IMAGE_SCN_MEM_DISCARDABLE
	...
	if(stab_section >= 0) {
		...
		memcpy(&output_image[secmap_pos + (int)sizeof(COFF_IMAGE_SECTION_HEADER) * stab_section],
			&stab, sizeof(stab));
	}
...
//		stabstr.Misc.VirtualSize = stabstr.SizeOfRawData = groups[GRP_COUNT].rva - groups[GRP_STABSTR].rva;
		...
		memcpy(&output_image[secmap_pos + (int)sizeof(COFF_IMAGE_SECTION_HEADER) * stabstr_section],
	...
	if(symbol_section >= 0) {
		...
		memcpy(&output_image[secmap_pos + (int)sizeof(COFF_IMAGE_SECTION_HEADER) * symbol_section],
			&sym, sizeof(sym));
	}
	...
	if(reloc_section >= 0) { // fill in relocation segment header
		...
		memcpy(&output_image[secmap_pos + (int)sizeof(COFF_IMAGE_SECTION_HEADER) * reloc_section],
			&reloc, sizeof(reloc));
	}
...
//	opthdr.SizeOfImage = (code_rva + groups[GRP_COUNT].rva + output_fixups.GetCount() + file_align - 1) & -file_align;
	...
	memcpy(&output_image[iohdr_pos], &opthdr, sizeof(opthdr));
...
//		| COFF_IMAGE_FILE_BYTES_REVERSED_LO
		...
		;
	memcpy(&output_image[ifhdr_pos], &ifhdr, sizeof(ifhdr));
...
void LinkJob::WriteImageFile()
{
	...
	fo.Put(output_image.Begin(), output_image.GetCount());
	if(stab_section >= 0) {
...
//	OpenCacheFile();
	...
	for(int i = 0; i < objects.GetCount(); i++) {
		...
		if(of.dll_atom)
			...
			const byte *begin = 0;
/*
			Buffer<byte> cachebuf;
			String cache_key;
			if(cache_objects)
				cache_key << of.library_file << ':' << of.object_file;
			int f;
			if(cache_objects && !cache_file.IsError() && (f = cache_name_index.Find(cache_key)) >= 0) {
				Cache& cache = object_cache[f];
				ASSERT(cache.library == of.library_file);
				ASSERT(cache.file == of.object_file);
				if(cache.time == of.file_time && cache.size == of.object_size) {
					LTIMING("reading cache data");
					cachebuf.Alloc(cache.size);
					cache_file.Seek(cache.offset);
					if(!cache_file.GetAll(cachebuf, cache.size))
						cache_file.SetError();
					if(!cache_file.IsError())
						begin = cachebuf;
				}
					...
					if(IsNull(of.library_file))
						...
						for(int r = cache_library_index.Find(of.library_file); r >= 0; r = cache_library_index.FindNext(r))
							to_remove.Add(r);
					while(!to_remove.IsEmpty()) {
						int i = to_remove.Pop();
						cache_name_index.Remove(i);
						cache_library_index.Remove(i);
						object_cache.Remove(i);
					}
				}
			}
*/
			if(!begin && !of.object_data.IsEmpty())
			...
			if(!begin) {
				...
				begin = mapping.Begin() + of.library_offset;
/*
				if(cache_objects && !cache_file.IsError()) {
					int first_pos = 36;
					int i = 0;
					while(i < object_cache.GetCount() && object_cache[i].offset - first_pos < of.object_size) {
						first_pos = object_cache[i].offset + object_cache[i].size;
						i++;
					}
					if(i && i >= object_cache.GetCount())
						first_pos += object_cache.Top().size >> 3;
					if(first_pos + of.object_size < CACHE_MAX_SIZE) {
						if(!cache_dirty) {
							if(cache_file.GetSize() < CACHE_TAG_SIZE) {
								cache_file.Seek(0);
								cache_file.Put(cachefile_header, CACHE_TAG_SIZE);
							}
							cache_file.Seek(CACHE_TAG_SIZE);
							cache_file.PutIL(0); ASSERT(CACHE_DIROFF_SIZE == 4);
							cache_dirty = true;
						}
						if((unsigned)first_pos > cache_file.GetSize()) {
							cache_file.SeekEnd();
							cache_file.Put('\0', int(first_pos - cache_file.GetPos()));
						}
						else
							cache_file.Seek(first_pos);
						cache_file.Put(begin, of.object_size);
						Cache& new_item = object_cache.Insert(i);
						new_item.library = of.library_file;
						new_item.file = of.object_file;
						new_item.time = of.file_time;
						new_item.offset = first_pos;
						new_item.size = of.object_size;
						cache_name_index.Insert(i, cache_key);
						cache_library_index.Insert(i, new_item.library);
					}
				}
*/
			}
			LTIMING("relocating objects");
			RelocateObject(of, begin);
			if(debug_info)
...
void LinkJob::RelocateImport(const ObjectFile& of)
{
	...
	int namesec_rva = (namesec.sec_map_index >= 0 ? section_map[namesec.sec_map_index].rva + code_rva : 0);
...
void LinkJob::RelocateInternal(const ObjectFile& of)
{
...
void LinkJob::RelocateObject(const ObjectFile& of, const byte *begin)
{
	...
	for(int s = section_object_index.Find(of.index); s >= 0; s = section_object_index.FindNext(s)) {
		...
		if(ofsec.offset)
			memcpy(sbegin, begin + ofsec.offset, ofsec.raw_size);
		...
		for(; relptr < relend; relptr++) {
			int delta_rva = relptr->VirtualAddress - secdata.VirtualAddress;
			int target_rva = destseg_rva + delta_rva;
			byte *target = &output_image[(int)(code_rfa + sec.rfa + delta_rva)];
			int value = 0;
			ObjSec obj_sec = Null;
			const COFF_IMAGE_SYMBOL& relsym = symtbl[relptr->SymbolTableIndex];
			int ref_atom = NameAtom(symtbl[relptr->SymbolTableIndex], strtbl);
			if(relsym.SectionNumber < 0) {
				value = relsym.Value;
				obj_sec.section = relsym.SectionNumber;
			}
			else if(relsym.StorageClass == COFF_IMAGE_SYM_CLASS_EXTERNAL
			|| relsym.StorageClass == COFF_IMAGE_SYM_CLASS_WEAK_EXTERNAL
//			|| relsym.SectionNumber > 0 && of.sections[relsym.SectionNumber].flags & COFF_IMAGE_SCN_LNK_COMDAT
			) {
				int coll = collected_symbols.Find(ref_atom);
				if(coll < 0 || collected_symbols[coll] < 0) {
					if(relsym.StorageClass == COFF_IMAGE_SYM_CLASS_WEAK_EXTERNAL) {
						for(int f = weak_externals.Find(ref_atom); f >= 0; f = weak_externals.FindNext(f)) {
							int c = collected_symbols.Find(weak_externals[f]);
							if(c >= 0 && collected_symbols[c] >= 0) {
								coll = c;
								break;
							}
						}
					}
					unresolved.FindAdd(ref_atom);
					continue;
				}
				...
				int sec_map_index = of.sections[relsym.SectionNumber].sec_map_index;
				if(sec_map_index >= 0) {
					...
					PutStdOut(NFormat("%s: invalid static relocation to %s (%d)",
	...
	if(debug_info) {
		...
		for(const COFF_IMAGE_SYMBOL *symp = symtbl; symp < symend; symp++) {
			...
			else if(AddCOFFSymbol(COFFSymbolName(*symp, strtbl), of.index, *symp))
...
void LinkJob::RelocateStabs(const ObjectFile& of, const byte *begin)
{
...
//			int secx = GetSection0(sym.obj_sec).sec_map_index;
			...
			if(secx >= 0) {
				...
				section_rfa = (secbegin.rfa < 0 ? -1 : secbegin.rfa + code_rfa);
	//			delta_rva = code_rva;
	//			delta_rfa = code_rfa + section_rfa;
		...
		map << NFormat("%7<s %08x %08x %08x %12<s %12<s %s\n",
			section, value - section_rva, value, (section_rfa < 0 ? -1 : value - section_rva + section_rfa),
...
void LinkJob::BuildLibrary()
{
	int start = msecs();
	ArchiveJob job;
	job.machine = machine;
	job.keep_object_paths = true;
	job.use_existing_archive = false;
	job.archive_must_exist = false;
	//job.trim_longname_objects;
	job.verbose = verbose;
	String msg;
	outputfile = AppendExt(outputfile, ".lib");
	String msgline = NFormat("Writing library '%s'... ", outputfile);
	for(int i = OBJ_FIRST; i < objects.GetCount(); i++) {
		ObjectFile& of = objects[i];
		if(of.dll_atom || !IsNull(of.library_file))
			continue;
	...
	if(dump_flags & DUMP_TIMING)
		PutStdOut(NFormat("Archived objects collected in %d msecs.", msecs(start)));

	if(!IsNull(msgline))
		msg << msgline << '\n';
	start = msecs();
	for(int i = 0; i < globals.GetCount(); i++) {
		const Symbol& glo = globals[i];
		int ai;
		if(glo.type < COFF_IMAGE_SYM_TYPE_SPECIAL && glo.external
		&& glo.obj_sec.section > 0 && glo.obj_sec.object >= 0
		&& (ai = objects[glo.obj_sec.object].archive_index) >= 0)
			job.AddGlobal(ai, atoms[globals.GetKey(i)]);
	}
	if(dump_flags & DUMP_TIMING)
		PutStdOut(NFormat("Global symbols collected in %d msecs.", msecs(start)));
	if(verbose)
		PutStdOut(msg);
	start = msecs();
	job.BuildLibrary(outputfile, false);
	if(dump_flags & DUMP_TIMING)
...
void LinkJob::Link()
{
	...
	if(dump_flags & DUMP_COMMANDLINE) {
		...
		for(int i = 0; i < user_command_args.GetCount(); i++)
			if(!IsNull(user_command_args[i]))
				PutStdOut(AsCString(user_command_args[i] + ' '));
