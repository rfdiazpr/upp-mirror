...
private:
	...
	static TabCtrl::Style leftstyle, rightstyle, bottomstyle;
	const TabCtrl::Style *style[4];
	
	void DrawTab(Draw &w, const TabCtrl::Style &s, const Size &sz, int i);
	int  TabPos(const String &g, bool &first, int i, int j);	
	...
	static int 	GetStyleHeight(const TabCtrl::Style& s);
...
protected:	
	...
	const TabCtrl::Style &GetStyle() { return *style[GetAlign()]; }
...
public:
	...
	TabBar& NoCursor(bool s = true) 	{ nosel = s; Refresh(); return *this; }
	...
	TabBar& SetStyle(int align, const TabCtrl::Style& s)  	{ ASSERT(align >= 0 && align < 4); style[align] = &s; Refresh(); return *this; }
	TabBar& SetStyles(const TabCtrl::Style& top);
	static const TabCtrl::Style& StyleDefault() 			{ return TabCtrl::StyleDefault(); }
	static const TabCtrl::Style& StyleLeft();
	static const TabCtrl::Style& StyleRight();
	static const TabCtrl::Style& StyleBottom();
	static const TabCtrl::Style& AlignStyle(int align, TabCtrl::Style &s);	
...
protected:
	...
	virtual void RightDown(Point p, dword keyflags) { if (GetHighlight() >= 0) WhenContext(GetHighlight()); }
